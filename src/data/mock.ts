import type { Campaign, Trigger, Template, BlacklistEntry, Customer, Segment } from '../types'

export const mockCampaigns: Campaign[] = [
  {
    id: '1',
    name: 'Welcome eSIM Q2/2026',
    code: 'CVM-WELCOME-ESIM-Q2-2026',
    status: 'Draft',
    triggers: ['SIM_ACTIVATED', 'NO_APP_INSTALL_24H'],
    templateIds: ['1', '5'],
    startDate: '01/04/2026',
    endDate: '30/06/2026',
    priority: 1,
    owner: 'QTV Marketing',
    createdAt: '10/05/2026 14:32',
    submittedAt: '12/05/2026 09:15',
    goal: 'Onboard eSIM, nhắc cài app',
  },
  {
    id: '2',
    name: 'Nhắc nạp tiền',
    code: 'CVM-REMIND-TOPUP-05-2026',
    status: 'Active',
    triggers: ['TOP_UP_FAIL'],
    templateIds: ['2'],
    startDate: '01/05/2026',
    endDate: '31/05/2026',
    priority: 2,
    owner: 'QTV Sales',
    createdAt: '28/04/2026 10:00',
  },
  {
    id: '3',
    name: 'Hết hạn gói data',
    code: 'CVM-DATA-EXPIRE-05-2026',
    status: 'Paused',
    triggers: ['PKG_EXPIRED'],
    templateIds: ['3'],
    startDate: '01/05/2026',
    endDate: '31/05/2026',
    priority: 3,
    owner: 'QTV Marketing',
    createdAt: '28/04/2026 11:00',
  },
  {
    id: '4',
    name: 'Chào mừng du lịch',
    code: 'CVM-TRAVEL-PROMO-Q2',
    status: 'Pending',
    triggers: ['LOC_TRAVEL_PROV'],
    templateIds: ['1', '4'],
    startDate: '15/05/2026',
    endDate: '30/06/2026',
    priority: 4,
    owner: 'QTV Marketing',
    createdAt: '14/05/2026 08:00',
    submittedAt: '14/05/2026 09:30',
  },
  {
    id: '7',
    name: 'Ưu đãi gói data sinh viên',
    code: 'CVM-STUDENT-DATA-Q2-2026',
    status: 'Pending',
    triggers: ['SIM_ACTIVATED'],
    templateIds: ['1'],
    startDate: '20/05/2026',
    endDate: '31/08/2026',
    priority: 7,
    owner: 'QTV Marketing',
    createdAt: '19/05/2026 10:15',
    submittedAt: '19/05/2026 14:00',
    goal: 'Tăng tỉ lệ đăng ký gói data cho phân khúc sinh viên',
  },
  {
    id: '8',
    name: 'Nhắc gia hạn gói cước tháng 6',
    code: 'CVM-RENEW-PKG-06-2026',
    status: 'Pending',
    triggers: ['PKG_EXPIRED'],
    templateIds: ['3'],
    startDate: '25/05/2026',
    endDate: '30/06/2026',
    priority: 8,
    owner: 'QTV Sales',
    createdAt: '20/05/2026 08:00',
    submittedAt: '20/05/2026 11:45',
  },
  {
    id: '9',
    name: 'Khuyến mãi nạp tiền đầu tháng',
    code: 'CVM-TOPUP-PROMO-06-2026',
    status: 'Pending',
    triggers: ['TOP_UP_FAIL'],
    templateIds: ['2'],
    startDate: '01/06/2026',
    endDate: '07/06/2026',
    priority: 9,
    owner: 'QTV Sales',
    createdAt: '21/05/2026 09:00',
    submittedAt: '21/05/2026 15:20',
    goal: 'Kích hoạt lại KH chưa nạp tiền trong 7 ngày',
  },
  {
    id: '10',
    name: 'Cài app nhận quà',
    code: 'CVM-APP-GIFT-06-2026',
    status: 'Pending',
    triggers: ['NO_APP_INSTALL_24H'],
    templateIds: ['5'],
    startDate: '01/06/2026',
    endDate: '15/06/2026',
    priority: 10,
    owner: 'QTV Marketing',
    createdAt: '21/05/2026 13:30',
    submittedAt: '22/05/2026 08:00',
    goal: 'Tăng tỉ lệ cài app trong 24h sau kích hoạt SIM',
  },
  {
    id: '6',
    name: 'Tết Nguyên Đán 2026',
    code: 'CVM-TET-2026-0001',
    status: 'Ended',
    triggers: ['SIM_ACTIVATED', 'TOP_UP_FAIL'],
    templateIds: ['1', '2', '4'],
    startDate: '20/01/2026',
    endDate: '10/02/2026',
    priority: 6,
    owner: 'QTV Marketing',
    createdAt: '15/01/2026 09:00',
    goal: 'Chúc Tết, khuyến mãi gói cước',
  },
  {
    id: '5',
    name: 'Giữ chân KH có nguy cơ rời mạng',
    code: 'CVM-RETENTION-Q2-2026',
    status: 'Active',
    triggers: ['SIM_NO_TXN_30D', 'USAGE_DROP_40PCT', 'LOW_DATA_BALANCE', 'NO_APP_INSTALL_24H', 'LOC_TRAVEL_PROV', 'TOP_UP_FAIL'],
    templateIds: ['2', '3', '5'],
    startDate: '01/05/2026',
    endDate: '30/06/2026',
    priority: 5,
    owner: 'QTV Marketing',
    createdAt: '13/05/2026 09:00',
    goal: 'Giảm churn rate, tăng engagement',
  },
]

export const mockTriggers: Trigger[] = [
  {
    id: '1',
    code: 'SIM_ACTIVATED',
    name: 'Kích hoạt SIM thành công',
    source: 'BSS',
    type: 'Realtime',
    status: 'Active',
    description: 'Fired khi KH kích hoạt SIM mới thành công',
    params: [
      { name: 'ten_kh', description: 'Họ tên đầy đủ của khách hàng', format: 'text', source: 'BSS' },
      { name: 'loai_sim', description: 'Loại SIM: eSIM hoặc vật lý', format: 'text', source: 'BSS' },
      { name: 'ngay_kich_hoat', description: 'Ngày kích hoạt SIM', format: 'date', source: 'BSS' },
      { name: 'so_dt', description: 'Số điện thoại', format: 'text', source: 'BSS' },
    ],
  },
  {
    id: '2',
    code: 'LOC_TRAVEL_PROV',
    name: 'Đến tỉnh du lịch',
    source: 'OCS',
    type: 'Realtime',
    status: 'Active',
    description: 'Fired khi KH đến vùng du lịch mới',
    params: [
      { name: 'tinh_thanh', description: 'Tên tỉnh/thành', format: 'text', source: 'OCS' },
      { name: 'thoi_gian', description: 'Thời điểm phát hiện', format: 'date', source: 'OCS' },
    ],
  },
  {
    id: '3',
    code: 'NO_APP_INSTALL_24H',
    name: 'Chưa cài app sau 24h',
    source: 'OCS',
    type: 'Near Realtime',
    status: 'Active',
    description: 'Fired sau 24h nếu KH chưa cài app',
    params: [
      { name: 'app_installed', description: 'Trạng thái cài app', format: 'boolean', source: 'OCS' },
      { name: 'last_active', description: 'Lần cuối hoạt động', format: 'date', source: 'OCS' },
    ],
  },
  {
    id: '4',
    code: 'LOW_DATA_BALANCE',
    name: 'Data còn dưới 100MB',
    source: 'OCS',
    type: 'Offline',
    status: 'Active',
    description: 'Batch job phát hiện KH sắp hết data',
    params: [
      { name: 'data_con_lai', description: 'Lượng data còn lại (MB)', format: 'number', source: 'OCS' },
      { name: 'ngay_het_han', description: 'Ngày hết hạn gói', format: 'date', source: 'OCS' },
    ],
  },
  {
    id: '5',
    code: 'USAGE_DROP_40PCT',
    name: 'Usage giảm >40%',
    source: 'OCS',
    type: 'Offline',
    status: 'Inactive',
    description: 'Phát hiện KH giảm sử dụng đột ngột',
    params: [
      { name: 'pct_drop', description: 'Phần trăm giảm', format: 'number', source: 'OCS' },
    ],
  },
  {
    id: '6',
    code: 'SIM_NO_TXN_30D',
    name: 'SIM ngừng GD >30 ngày',
    source: 'BSS',
    type: 'Offline',
    status: 'Inactive',
    description: 'KH không có giao dịch trong 30 ngày',
    params: [
      { name: 'so_ngay', description: 'Số ngày không giao dịch', format: 'number', source: 'BSS' },
    ],
  },
]

export const mockTemplates: Template[] = [
  { id: '1', name: 'Chào mừng SIM', channels: ['Push', 'Zalo OA'], usageCount: 3, status: 'Active' },
  { id: '2', name: 'Nhắc nạp thẻ', channels: ['SMS', 'USSD'], usageCount: 3, status: 'Active' },
  { id: '3', name: 'Sắp hết data', channels: ['Push', 'SMS'], usageCount: 2, status: 'Active' },
  { id: '4', name: 'Sinh nhật KH', channels: ['Zalo OA', 'Email'], usageCount: 2, status: 'Active' },
  { id: '5', name: 'Cài app nhắc nhở', channels: ['Push'], usageCount: 2, status: 'Inactive' },
]

export const mockBlacklist: BlacklistEntry[] = [
  { phone: '0987654001', campaign: 'Nhắc nạp tiền',              channel: 'Push',    source: 'campaign' },
  { phone: '0912345002', campaign: 'Nhắc nạp tiền',              channel: 'SMS',     source: 'upload' },
  { phone: '0965432003', campaign: 'Nhắc nạp tiền',              channel: 'Zalo OA', source: 'manual' },
  { phone: '0976543004', campaign: 'Welcome eSIM Q2/2026',        channel: 'Push',    source: 'campaign' },
  { phone: '0933221005', campaign: 'Welcome eSIM Q2/2026',        channel: 'Zalo OA', source: 'upload' },
  { phone: '0901122006', campaign: 'Welcome eSIM Q2/2026',        channel: 'Email',   source: 'manual' },
  { phone: '0918765007', campaign: 'Hết hạn gói data',           channel: 'SMS',     source: 'upload' },
  { phone: '0977889008', campaign: 'Hết hạn gói data',           channel: 'USSD',    source: 'upload' },
  { phone: '0944556009', campaign: 'Hết hạn gói data',           channel: 'Push',    source: 'campaign' },
  { phone: '0908877010', campaign: 'Chào mừng du lịch',          channel: 'Banner',  source: 'manual' },
  { phone: '0961234011', campaign: 'Chào mừng du lịch',          channel: 'Zalo OA', source: 'campaign' },
  { phone: '0923456012', campaign: 'Giữ chân KH có nguy cơ rời mạng', channel: 'SMS', source: 'upload' },
  { phone: '0934567013', campaign: 'Giữ chân KH có nguy cơ rời mạng', channel: 'Push', source: 'campaign' },
  { phone: '0945678014', campaign: 'Giữ chân KH có nguy cơ rời mạng', channel: 'Email', source: 'manual' },
  { phone: '0956789015', campaign: 'Tết Nguyên Đán 2026',        channel: 'Banner',  source: 'upload' },
]

export const mockCustomers: Customer[] = [
  { phone: '0987 xxx 001', simType: 'eSIM', status: 'Active', hasApp: true, hasDnc: false },
  { phone: '0912 xxx 002', simType: 'SIM vật lý', status: 'Active', hasApp: false, hasDnc: false },
  { phone: '0965 xxx 003', simType: 'eSIM', status: 'Inactive', hasApp: true, hasDnc: true },
  { phone: '0976 xxx 004', simType: 'SIM vật lý', status: 'Active', hasApp: false, hasDnc: false },
]

export const mockSegments: Segment[] = [
  { id: '1', name: 'Gen Z User (18–25)', reach: 18450, source: 'Customer 360' },
  { id: '2', name: 'Sắp hết data', reach: 8920, source: 'OCS' },
  { id: '3', name: 'ARPU cao', reach: 5600, source: 'BSS' },
  { id: '4', name: 'Nguy cơ rời mạng', reach: 2150, source: 'BSS' },
  { id: '5', name: 'VIP', reach: 3200, source: 'CRM' },
]

export const recentTriggerEvents = [
  { time: '09:44:52', code: 'LOW_DATA_BALANCE', phone: '0987 xxx 001', result: 'Matched · Push sent', type: 'success' },
  { time: '09:44:38', code: 'SIM_ACTIVATED', phone: '0912 xxx 002', result: 'Matched · Zalo sent', type: 'success' },
  { time: '09:44:21', code: 'NO_APP_INSTALL_24H', phone: '0965 xxx 003', result: 'No match', type: 'neutral' },
  { time: '09:44:05', code: 'TOP_UP_FAIL', phone: '0976 xxx 004', result: 'Blacklist blocked', type: 'blocked' },
  { time: '09:43:58', code: 'LOW_DATA_BALANCE', phone: '0988 xxx 005', result: 'Matched · DNC blocked', type: 'blocked' },
]
